    ...
    return {
        userPosts: function(userId, callback) {
            ...
            request(url, function(error, response, body) {
                ...
                if (json.error) {
                    var error = function() {
                        if (json.error.code == 404) return new errors.NotFoundError(json.error.message)
                        if (json.error.code >= 400) return new errors.UserError(json.error.message)
                        return new errors.ServerError(json.error.message, "Google+ error: " + json.error.message)
                    }()
                    return callback(error)
                }
                var posts = []
